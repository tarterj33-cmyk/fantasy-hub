...
function FreeAgents(p){ const state=p.state; const [q,setQ]=useState(""); const [pos,setPos]=useState(""); const [sort,setSort]=useState("rank"); const posList=["QB","RB","WR","TE","DST","K"]; function filtered(){ let arr=freeAgentList(state).filter(function(pl){ return (!q||pl.name.toLowerCase().includes(q.toLowerCase())) && (!pos||pl.pos===pos); }); arr=arr.sort(function(a,b){ if(sort==='rank') return (a.rank||9999)-(b.rank||9999) || a.name.localeCompare(b.name); if(sort==='adp') return (a.adp||9999)-(b.adp||9999) || a.name.localeCompare(b.name); if(sort==='name') return a.name.localeCompare(b.name); if(sort==='proj') return (b.projected||0)-(a.projected||0) || a.name.localeCompare(b.name); return 0; }); return arr; } const rows=filtered(); return (<Card><CardHeader title="Free Agents" sub="View-only list from your CSV; commissioner assigns players under Roster Editor"/><CardBody><div className="grid md:grid-cols-4 gap-2 items-end"><div className="md:col-span-2"><div className="text-xs text-gray-500 mb-1">Search</div><Input placeholder="Name" value={q} onChange={function(e){setQ(e.target.value);}}/></div><div><div className="text-xs text-gray-500 mb-1">Position</div><Select value={pos} onChange={function(e){setPos(e.target.value);}}><option value="">All</option>{posList.map(function(pp){return (<option key={pp} value={pp}>{pp}</option>);})}</Select></div><div><div className="text-xs text-gray-500 mb-1">Sort</div><Select value={sort} onChange={function(e){setSort(e.target.value);}}><option value="rank">Rank</option><option value="adp">ADP</option><option value="name">Name</option><option value="proj">Projected</option></Select></div></div><div className="mt-3 rounded-2xl border overflow-auto"><table className="min-w-full text-sm"><thead className="text-left text-xs text-gray-500"><tr><th className="p-2">Rank</th><th className="p-2">Player</th><th className="p-2">Pos</th><th className="p-2">Team</th><th className="p-2">Bye</th><th className="p-2">ADP</th><th className="p-2">Projected</th></tr></thead><tbody>{rows.map(function(pl){ return (<tr key={pl.id} className="border-b last:border-0"><td className="p-2">{pl.rank!=null?pl.rank:''}</td><td className="p-2 font-medium">{pl.name}</td><td className="p-2">{pl.pos}</td><td className="p-2">{pl.nfl}</td><td className="p-2">{pl.bye!=null?pl.bye:''}</td><td className="p-2">{pl.adp!=null?pl.adp:''}</td><td className="p-2">{formatNum(pl.projected||0)}</td></tr>); })}{rows.length===0?(<tr><td className="p-2 text-sm text-gray-500" colSpan={7}>No players match.</td></tr>):null}</tbody></table></div></CardBody></Card>); }
